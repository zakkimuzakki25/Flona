...
import PopupConfirmation from "../../components/common/popup/PopupConfirmation";
import PopupSucces from "../../components/common/popup/PopupSucces";
...
const PaymentInstruction = () => {
  ...
  return (
    <div className="bg-onyx">
      {isLeft && (
        <PopupConfirmation
          heading={"Apakah kamu yakin Ingin MEMBATALKAN pembayaran?"}
          nameButton={"ok"}
          nameButtonNo={"TIDAK"}
          nameButtonYes={"IYA"}
          handlerButtonNo={() => setIsLeft(false)}
          handlerButtonYes={confirmLeave}
      ...
      {isDone && (
        <PopupSucces
          ...
          body={`Terima kasih, sobat Flonn! Donasimu sebesar ${new Intl.NumberFormat("id-ID", {style: "currency",currency: "IDR",minimumFractionDigits: 0,maximumFractionDigits: 0,}).format(data.amount)} sudah kami terima dan akan disalurkan.`}
          nameButton={"KEMBALI KE HALAMAN DONASI"}
          handlerButton={() => {
            setIsDone(false);
      ...
      <div className="h-fit w-full flex flex-row px-40 py-2.5 gap-5 lg:mt-28">
        ...
        <button
          onClick={() =>
            handleNavigate(`/aksi/donasi/${data.open_donation_id}`)
          }
          className="bl text-white hover:text-cambridgeBlue"
        >
          Donasi korban kebakaran hutan di Ohio
        </button>
        <p className="bl text-white">&gt;</p>
        <button
          onClick={() =>
            handleNavigate(`/aksi/donasi/${data.open_donation_id}/pembayaran`)
          }
          className="bl text-white hover:text-cambridgeBlue"
        >
          Metode Pembayaran
        ...
        <div className="flex w-full flex-col bg-viridian rounded-3xl">
          ...
          <div className="flex flex-col bg-white rounded-3xl px-9 py-10 gap-5">
            ...
            <div className="flex flex-row justify-between">
              <p className="ds">
                {getSelectedMethod("Mandiri Virtual Account").nama}
              </p>
            ...
            <div className="flex flex-row justify-between bg-abu px-4 items-center py-5 rounded-2xl">
              <p style={{ lineHeight: "1" }} className="ds">
                xxxxxxxxxxxxxxxx
              </p>
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-row self-end w-72 mt-2">
                <PrimerButton
            ...
            <div className="h-0.5 w-full bg-abu"></div>
            {/* Panduan pembayaran */}
            {getSelectedMethod("Mandiri Virtual Account").instructions &&
              getSelectedMethod("Mandiri Virtual Account")
                .instructions.split("<>")
                .map((item, index) => {
                  return (
